    ...
    handler: async (_args, ctx) => {
      ...
      ctx.ui.notify(
        [
          ...
          "Real backends: TypeScript language service for javascript/typescript, basedpyright prototype for python, and a bash-language-server plus ShellCheck prototype for bash. The TS path supports hover, explain-symbol, definitions, document highlights, diagnostics, and semantic-token payloads. Python currently supports diagnostics, hover, explain-symbol, definitions, and semantic-token payloads. Bash currently supports diagnostics, hover-like explain, definitions, and document highlights through the prototype path; upstream bash-language-server still does not expose semantic tokens, but the host now supplies a separate Bash smarter-highlighting seam for normal tool-row rendering.",
