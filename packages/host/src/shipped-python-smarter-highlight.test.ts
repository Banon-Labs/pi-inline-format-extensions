...
type StyleBucket =
  (typeof SHIPPED_PYTHON_SMARTER_HIGHLIGHT_STYLE_BUCKETS)[number][number];
...
test("pins the shipped Python smarter-highlighted tool-row output", () => {
  ...
  const expectedBodyLines = SHIPPED_PYTHON_SMARTER_HIGHLIGHT_SEGMENTS.map(
    (segments, index) =>
      renderExpectedLine(
        segments,
        SHIPPED_PYTHON_SMARTER_HIGHLIGHT_STYLE_BUCKETS[index]!,
      ),
  );
