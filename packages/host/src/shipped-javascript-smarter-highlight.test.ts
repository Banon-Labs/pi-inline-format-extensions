...
import { STANDARD_JAVASCRIPT_SAMPLE_COMMAND } from "./demo-samples.js";
import {
...
type StyleBucket =
  (typeof SHIPPED_JAVASCRIPT_SMARTER_HIGHLIGHT_STYLE_BUCKETS)[number][number];
...
test("pins the shipped JavaScript smarter-highlighted tool-row output", () => {
  ...
  const match = detectInlineFormatMatches(
    STANDARD_JAVASCRIPT_SAMPLE_COMMAND,
  ).find((candidate) => candidate.language === "javascript");
  assert.ok(match, "expected the shipped JavaScript sample to be detected");

  const actualBodyLines = rendered
    .render(400)
    .map((line) => stripAnsi(line).trimEnd())
    .slice(match.startLineIndex, match.endLineIndex + 1);
  const expectedBodyLines = SHIPPED_JAVASCRIPT_SMARTER_HIGHLIGHT_SEGMENTS.map(
    (segments, index) =>
      renderExpectedLine(
        segments,
        SHIPPED_JAVASCRIPT_SMARTER_HIGHLIGHT_STYLE_BUCKETS[index]!,
      ),
  );
