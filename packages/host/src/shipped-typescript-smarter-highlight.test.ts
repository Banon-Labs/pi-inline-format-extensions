...
type StyleBucket =
  (typeof SHIPPED_TYPESCRIPT_SMARTER_HIGHLIGHT_STYLE_BUCKETS)[number][number];
...
test("pins the shipped TypeScript smarter-highlighted tool-row output", () => {
  ...
  const match = detectInlineFormatMatches(
    SHIPPED_TYPESCRIPT_SAMPLE_COMMAND,
  ).find((candidate) => candidate.language === "typescript");
  assert.ok(match, "expected the shipped TypeScript sample to be detected");

  const actualBodyLines = rendered
    .render(400)
    .map((line) => stripAnsi(line).trimEnd())
    .slice(match.startLineIndex, match.endLineIndex + 1);
  const expectedBodyLines = SHIPPED_TYPESCRIPT_SMARTER_HIGHLIGHT_SEGMENTS.map(
    (segments, index) =>
      renderExpectedLine(
        segments,
        SHIPPED_TYPESCRIPT_SMARTER_HIGHLIGHT_STYLE_BUCKETS[index]!,
      ),
  );
